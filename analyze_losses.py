...
for di,dt in enumerate(cd):
    for code,p in list(pos.items()):
        ...
        if px<=p['peak']*(1-TRAIL):
            ...
            # Get daily prices from entry to exit for drawdown path
            dd_path=[]
            for k in range(p['bi'],di+1):
                dk=cd[k];pxk=stocks[code]['close'][idx[code][dk]]
                dd_k=(pxk-p['bp'])/p['bp']*100
                dd_path.append(dd_k)
            trades.append({'code':code,'name':stocks[code]['name'],
                ...
                'peak_day':p.get('peak_day',0)-p['bi'],
    ...
    if di%REBAL==0:
        ...
        for code in list(pos.keys()):
            if code not in top_codes:
                ...
                dd_path=[]
                for k in range(pos[code]['bi'],di+1):
                    dk=cd[k];pxk=stocks[code]['close'][idx[code][dk]]
                    dd_path.append((pxk-pos[code]['bp'])/pos[code]['bp']*100)
                trades.append({'code':code,'name':stocks[code]['name'],
                    ...
                    'hold':di-pos[code]['bi'],'dd_path':dd_path,
                    'peak_day':pos[code].get('peak_day',0)-pos[code]['bi'],
        ...
        for code,score in selected:
            ...
            if cash<target_val*0.99: continue
...
print('='*85)
print(' 亏损深度分析 | K=1.5 LB=14 Trail=30% Strict-Sector')
print('='*85)
...
print(' 1. 亏损大小分布')
print('-'*85)
losses=sorted([t['ret']*100 for t in losers])
buckets=[(-100,-25),(-25,-20),(-20,-15),(-15,-10),(-10,-5),(-5,0)]
for lo,hi in buckets:
    cnt=sum(1 for l in losses if l>=lo and l<hi)
    bar='█'*cnt if cnt>0 else ''
    if cnt>0:
        avg=sum(l for l in losses if l>=lo and l<hi)/cnt
        print('  [%+3d%%,%+3d%%): %3d trades  avg=%+.1f%%  %s' % (lo,hi,cnt,avg,bar))

# ===== 2. TIME SERIES OF DRAWDOWNS =====
print('\n' + '-'*85)
print(' 2. 亏损的时间分布')
print('-'*85)
for y in ['2020','2021','2022','2023','2024','2025','2026']:
    ...
    if sub:
        ...
        worst=min(t['ret'] for t in sub)*100
        print('  %s: %2d losses / %2d wins  avg=%.1f%%  worst=%.1f%%  loss_rate=%.0f%%' % (
            y, len(sub), len(sub_w), avg, worst, len(sub)/(len(sub)+len(sub_w))*100))
...
# ===== 5. PEAK-TO-FINAL: HOW MANY WERE ONCE WINNERS? =====
print('\n' + '-'*85)
print(' 5. 曾经盈利但最终亏损 (peak>0% then crashed)')
print('-'*85)
...
for lo,hi in [(0,5),(5,10),(10,20),(20,50),(50,200)]:
    sub=[t for t in crashed if t.get('peak_ret',0)>=lo and t.get('peak_ret',0)<hi]
    if sub:
...
print(' 6. 亏损按持仓天数')
print('-'*85)
for lo,hi in [(0,5),(5,10),(10,21),(21,42),(42,100)]:
...
# ===== 7. CRASH PATTERN: same date, multiple losses =====
print('\n' + '-'*85)
print(' 7. 同日多笔亏损 (系统性崩盘日)')
print('-'*85)
losers.sort(key=lambda x:x['bd'])
date_losses=defaultdict(list)
for t in losers: date_losses[t['bd']].append(t)
clusters=[(d,trades) for d,trades in date_losses.items() if len(trades)>=2]
...
for d,cl in clusters[:8]:
    ...
    print('  %s: %d trades  total_loss=%.1f%%  %s' % (
        d, len(cl), total, ', '.join('%s(%.1f%%,%s)'%(t['name'][:6],t['ret']*100,t['exit'][:5]) for t in cl)))

# ===== 8. ENTRY FACTOR vs LOSS =====
print('\n' + '-'*85)
print(' 8. 入场因子值与亏损的关系')
print('-'*85)
valid_losers=[t for t in losers if not math.isnan(t.get('entry_factor',float('nan')))]
valid_winners=[t for t in winners if not math.isnan(t.get('entry_factor',float('nan')))]
for tag,data in [('Losers',valid_losers),('Winners',valid_winners)]:
    if data:
        fvs=[t['entry_factor'] for t in data]
        fvs.sort();n=len(fvs)
        print('  %-8s: min=%.2f p10=%.2f med=%.2f p90=%.2f max=%.2f' % (
            tag, fvs[0], fvs[n//10], fvs[n//2], fvs[n*9//10], fvs[-1]))

# ===== 9. NAV LEVEL (does market regime matter?) =====
print('\n' + '-'*85)
print(' 9. 不同净值水位下的亏损')
print('-'*85)
all_navs=sorted([t.get('nav_at_entry',INIT) for t in trades])
n=len(all_navs)
p33=all_navs[n//3];p66=all_navs[n*2//3]
for tag,nav_range in [('Low (<%.0f万)'%(p33/1e4),(0,p33)),
                       ('Mid','mid'),('High (>%.0f万)'%(p66/1e4),(p66,float('inf')))]:
    sub=[t for t in losers if (nav_range[0]<=t.get('nav_at_entry',INIT)<nav_range[1])]
    sub_all=[t for t in trades if (nav_range[0]<=t.get('nav_at_entry',INIT)<nav_range[1])]
    if nav_range==('mid','mid'):
        sub=[t for t in losers if p33<=t.get('nav_at_entry',INIT)<p66]
        sub_all=[t for t in trades if p33<=t.get('nav_at_entry',INIT)<p66]
    if sub:
        avg=sum(t['ret'] for t in sub)/len(sub)*100
        loss_rate=len(sub)/len(sub_all)*100 if sub_all else 0
        print('  %-25s: %3d losses  avg=%.1f%%  loss_rate=%.0f%%' % (tag,len(sub),avg,loss_rate))

# ===== 10. TREND AT ENTRY =====
print('\n' + '-'*85)
print(' 10. ALL LOSING TRADES (sorted worst first)')
print('-'*85)
