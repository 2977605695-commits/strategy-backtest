...
def main():
    ...
    for di,ds in enumerate(cd):
        ...
        for c,h in list(holdings.items()):
            ...
            if ext:
                ...
                events.append({
                    'date':ds,'type':'SELL','stock':pc['name'],'sector':h['sector'],
                    'buy_px':h['buy_px'],'sell_px':sp,'ret':ret,
                    'buy_date':h['buy_date'],'peak':h['peak'],'days':di-h['buy_day'],
                    'exit':ext,
                })
                trades.append({'name':pc['name'],'ret':ret,'exit':ext,'buy_date':h['buy_date'],'days':di-h['buy_day']})
                sells.append((c,nc))
        ...
        while len(holdings)<MP and elig and cash>=per:
            ...
            events.append({
                'date':ds,'type':'BUY','stock':pc['name'],'sector':sec,
    ...
    for c,h in list(holdings.items()):
        ...
        ret=(sp-h['buy_px'])/h['buy_px']
        events.append({
            'date':cd[-1],'type':'FINAL','stock':pc['name'],'sector':h['sector'],
            'buy_px':h['buy_px'],'sell_px':sp,'ret':ret,
            'buy_date':h['buy_date'],'days':len(cd)-1-h['buy_day'],
        })
    ...
    # ================================================================
    print("="*100)
    print(f"  F60d DEV<{-BUY_THR:.1%} Trail{TRAIL:.0%}  完整操作记录")
    print(f"  44股 1451天 2020-2026 | T+1+滑点+手续费")
    print("="*100)
    print(f"\n  绩效: {INIT:,.0f} -> {fv:,.0f} | +{tr*100:.1f}% | 年化{cagr*100:.1f}%")
    print(f"  夏普 {sh:.4f} | 回撤 {mdd*100:.1f}% | {len(trades)}笔交易 | "
          f"Trail={sum(1 for t in trades if t['exit']=='trail')} "
          f"Time={sum(1 for t in trades if t['exit']=='time')} "
          f"Final={sum(1 for t in trades if t['exit']=='final')}")
    ...
    print(f"{'='*100}")
    print(f"  {'#':<3s} {'股票':<12s} {'买入日':<12s} {'退出':<6s} {'收益':>8s} {'持天':>5s}")
    print(f"  {'-'*48}")
    wins=0; total_ret=0
    for i,t in enumerate(trades):
        ...
        if t['ret']>0: wins+=1
        total_ret+=t['ret']*100
        print(f"  {i+1:<3d} {t['name']:<12s} {t['buy_date']:<12s} {t['exit']:<6s} {t['ret']*100:>+7.1f}% {t['days']:>5d} {tag}")
    ...
    for year in [2020,2021,2022,2023,2024,2025,2026]:
        buys_yr=[e for e in events if e['type']=='BUY' and year==int(e['date'][:4])]
        sells_yr=[e for e in events if e['type']=='SELL' and year==int(e['date'][:4])]
        yr_dvs=[dv for dv in dvs if dv['date'][:4]==str(year)]
        if yr_dvs:
            yr_ret=(yr_dvs[-1]['value']/yr_dvs[0]['value']-1)*100
            print(f"  {year}: 买入{len(buys_yr)}次 卖出{len(sells_yr)}次  年度收益{yr_ret:+.1f}%")
            if buys_yr:
                names=set(e['stock'] for e in buys_yr)
                print(f"    买入: {', '.join(sorted(names))}")
        else:
            print(f"  {year}: (无数据)")
