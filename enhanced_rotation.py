"""
增强轮动策略: +MA5>MA20趋势 +基本面恶化止损 +60日时限
=========================================================
基础: 基本面评分轮动, 5只, 赛道不重复, T+1+滑点+手续费
新增:
  C: 买入需 MA5 > MA20（不接飞刀）
  D: 持仓股基本面得分恶化>1.0σ 强制卖出
  F: 持仓超60个交易日强制卖出（盘活资金）
Grid: DEV × Trail
"""
...
def backtest(buy_thr, trail_pct, stocks_data, fund_data, common_dates, enhanced=True):
    ...
    for code, info in stocks_data.items():
        ...
        ma20 = calc_ma(closes, MA_TREND)  # C: trend filter
        devs = []; trends = []
        for i, bar in enumerate(bars):
            ...
            if math.isnan(ma) or ma == 0:
                ...
                # C: MA5 > MA20 means uptrend
                trends.append(not math.isnan(ma20[i]) and ma5[i] > ma20[i])
        precomputed[code] = {
            'bars': bars, 'closes': closes, 'devs': devs, 'trends': trends,
    ...
    trail_exits = 0; fund_exits = 0; time_exits = 0; final_exits = 0
    ...
    for day_idx, date_str in enumerate(common_dates):
        ...
        # Check exits: Trail / Fundamental D / Time F
        sell_events = []
        for code, h in list(holdings.items()):
            ...
            if enhanced:
                # D: Fundamental deterioration
                if not exit_type and new_scores and code in new_scores and code in precomputed:
                    entry_score = h.get('entry_score', 0)
                    current_score = new_scores[code]
                    if current_score < entry_score - FUND_DETERIORATION:
                        exit_type = 'fundamental'

                # F: 60-day time limit
                if not exit_type and day_idx - h['buy_day'] >= MAX_HOLD_DAYS:
                    exit_type = 'time'
            ...
            if exit_type:
                ...
                trades.append({'name': pc['name'], 'ret': ret, 'buy_date': h['buy_date'],
                              'sell_date': date_str, 'exit': exit_type,
                              'days': day_idx - h['buy_day']})
                if exit_type == 'trail': trail_exits += 1
                elif exit_type == 'fundamental': fund_exits += 1
                elif exit_type == 'time': time_exits += 1
                sell_events.append((code, net_cash))

        for code, cr in sell_events: cash_pool += cr; del holdings[code]

        # Buy with C: trend filter
        ...
        for code, score in sorted(current_scores.items(), key=lambda x: x[1], reverse=True):
            ...
            dev = pc['devs'][day_idx]
            trend_ok = pc['trends'][day_idx] if enhanced else True
            # Must satisfy: DEV signal + trending up (C, if enhanced)
            if not math.isnan(dev) and dev < buy_thr and trend_ok:
        ...
        while len(holdings) < MAX_POSITIONS and eligible and cash_pool >= per_stock_cap:
            ...
            holdings[code] = {
                ...
                'sector': pc['sector'], 'entry_score': score,
            }
            cash_pool -= per_stock_cap; held_sectors.add(pc['sector'])
    ...
    for code, h in list(holdings.items()):
        ...
        ret = (sell_px - h['buy_px']) / h['buy_px']
        trades.append({'name': pc['name'], 'ret': ret, 'buy_date': h['buy_date'],
                      'sell_date': common_dates[-1], 'exit': 'final',
                      'days': len(common_dates)-1-h['buy_day']})
        final_exits += 1
    ...
    return {
        ...
        'trail_exits': trail_exits, 'fund_exits': fund_exits,
        'time_exits': time_exits, 'final_exits': final_exits,
        'trades': trades,
    }
...
def main():
    ...
    print(f"Enhancements: C=MA5>MA20  D=基本面恶化>1.0σ  F={MAX_HOLD_DAYS}日时限\n")

    buy_thresholds = [-0.035, -0.040, -0.045, -0.050, -0.055]
    trail_pcts = [0.20, 0.25, 0.30, 0.35, 0.40]
    ...
    for buy_thr in buy_thresholds:
        for trail_pct in trail_pcts:
            ...
            print(f"[{done:>2d}/{total}] DEV<{buy_thr:.1%} Tr{trail_pct:.0%} | "
                  ...
                  f"Trail={r['trail_exits']} Fund={r['fund_exits']} Time={r['time_exits']} "
                  f"Fin={r['final_exits']} Pos={r['avg_positions']:.1f}")

    sorted_all = sorted(all_results, key=lambda r: r['sh'], reverse=True)
    global_max = max(r['sh'] for r in all_results)

    # Also run BASELINE (no C,D,F) for best params
    print(f"\n\n  --- BASELINE (无C/D/F, 原版轮动) ---")
    for bt, tp, label in [(-0.055, 0.45, '原版进攻'), (-0.035, 0.35, '原版稳健')]:
        bl = backtest(bt, tp, qualified, fund_data, common_dates, enhanced=False)
        print(f"  {label} DEV<{bt:.1%} Tr{tp:.0%}: Sh={bl['sh']:.4f} Ret={bl['tr']*100:.0f}% DD={bl['mdd']*100:.1f}%")

    # ... but we can't easily run without CDF using the same backtest function
    # So let me show a comparison note

    print(f"\n\n{'='*110}")
    print(f"  增强策略排名 (C=MA5>MA20 D=恶化>1σ F={MAX_HOLD_DAYS}d)")
    print(f"{'='*110}")
    print(f"  {'#':<3s} {'策略':<26s} {'夏普':>7s} {'收益':>7s} {'年化':>6s} "
          f"{'回撤':>6s} {'卡玛':>6s} {'交易':>4s} {'胜率':>5s} "
          f"{'T/Fu/Ti/Fi':>10s} {'持仓':>4s}")
    print(f"  {'-'*100}")

    for rank, r in enumerate(sorted_all, 1):
        tag = " << MAX" if r['sh'] == global_max else ""
        exit_str = f"{r['trail_exits']}/{r['fund_exits']}/{r['time_exits']}/{r['final_exits']}"
        print(f"  {rank:<3d} DEV<{r['buy_thr']:.1%} Trail{r['trail_pct']:.0%}     "
              ...
              f"{exit_str:>10s} {r['avg_positions']:>4.1f}{tag}")
    ...
    for buy_thr in buy_thresholds:
        row = f"  DEV<{buy_thr:.1%}"
        for trail_pct in trail_pcts:
            m = [x for x in all_results if abs(x['buy_thr']-buy_thr)<0.001 and abs(x['trail_pct']-trail_pct)<0.001]
            if m:
                sh = m[0]['sh']
    ...
    print(f"\n\n  --- vs 原版轮动 (无C/D/F) ---")
    print(f"  原版最优: DEV<-5.5% Trail45%  Sharpe=1.30  Ret=647%  DD=23.4%")
    print(f"  增强最优: DEV<{sorted_all[0]['buy_thr']:.1%} Trail{sorted_all[0]['trail_pct']:.0%}  "
          f"Sharpe={sorted_all[0]['sh']:.4f}  Ret={sorted_all[0]['tr']*100:.0f}%  "
          f"DD={sorted_all[0]['mdd']*100:.1f}%")
    dsh = sorted_all[0]['sh'] - 1.30
    print(f"  dSharpe = {dsh:+.4f}")
    ...
    print(f"  {'退出':<14s} {'笔数':>5s} {'均收益':>8s} {'胜率':>6s}")
    for ext in ['trail', 'fundamental', 'time', 'final']:
        if ext in exit_types:
            et = exit_types[ext]
            avg_r = sum(et['rets'])/len(et['rets'])*100
            wr = sum(1 for r in et['rets'] if r>0)/len(et['rets'])*100
            print(f"  {ext:<14s} {et['count']:>5d} {avg_r:>+7.1f}% {wr:>5.0f}%")
