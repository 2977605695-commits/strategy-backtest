...
def bt(no_chase, vol_filter):
        ...
        if di%REBAL==0:
            ...
            if vol_filter:
                ...
                for c,s in cand:
                    ...
                    if si and si>=20:
                        rets=[]
                        for j in range(si-19,si+1):
                            if j>0 and stocks[c]['close'][j-1]>0:
                                rets.append(stocks[c]['close'][j]/stocks[c]['close'][j-1]-1)
                        if rets:
                            mu_r=sum(rets)/len(rets);vol20=(sum((r-mu_r)**2 for r in rets)/len(rets))**0.5
                            if vol20>vol_filter: continue
                    filtered.append((c,s))
                cand=filtered
    ...
    for x in v:
        if x>pk: pk=x
        dd_i=(pk-x)/pk
        if dd_i>mdd: mdd=dd_i
    cm=cagr/mdd if mdd>0 else float('inf')
    w=sum(1 for t in trades if t['ret']>0);nl=sum(1 for t in trades if t['ret']<0)
    # Count short-term losses eliminated
    short_losses=[t for t in trades if t['ret']<0 and t.get('hold',99)<21]
...
print('='*85)
print('  降MDD: 禁止追涨 + 波动率过滤')
print('  基准: K=1.5 LB=14 Trail=30% min_f=0.8 Strict-Sector')
print('='*85)

configs=[
    ('#0 Baseline',None,None),
    ('A1 NoChase>5%',0.05,None),('A2 NoChase>8%',0.08,None),
    ('A3 NoChase>10%',0.10,None),('A4 NoChase>15%',0.15,None),
    ('B1 Vol<3%',None,0.03),('B2 Vol<3.5%',None,0.035),('B3 Vol<4%',None,0.04),
    ('C1 Chase8%+Vol3.5%',0.08,0.035),('C2 Chase10%+Vol3.5%',0.10,0.035),
]

print('  %-25s %7s %8s %6s %6s %6s %5s %5s' % ('Config','Sharpe','Ret','CAGR','MDD','Calmar','Trd','Win'))
print('  '+'-'*70)
results={}
for label,nc,vf in configs:
    r=bt(nc,vf)
    results[label]=r
    tag=' *' if r['sh']==max(x['sh'] for x in results.values()) else ''
    tag2=' @' if r['mdd']==min(x['mdd'] for x in results.values()) else ''
    print('  %-25s %7.3f %7.1f%% %5.2f%% %5.1f%% %6.3f %4d %4.0f%%%s%s' % (
        label[:25],r['sh'],r['tr']*100,r['cagr']*100,r['mdd']*100,r['calmar'],
        r['nt'],r['wr']*100,tag,tag2))

base=results['#0 Baseline']
print('\n  vs Baseline:')
print('  %-25s %8s %8s %8s %8s' % ('Config','dSharpe','dRet%','dMDD%','dTrd'))
print('  '+'-'*52)
for label,r in results.items():
    ...
    print('  %-25s %+7.3f %+7.1f %+7.1f %+5d%s' % (label[:25],ds,dr,dd,dtrd,tag))
