"""No-surge filter: skip stocks that already rallied before buy signal"""
...
def backtest(trail,rebal,max_same_sector,max_pos,factor_floor,surge_days,surge_thresh):
    cash=INIT;pos={};eq=[];t_n=0;wins=0;surge_skip=0
    ...
    for di,dt in enumerate(cd):
        ...
        if di%rebal==0:
            ...
            # Surge filter: skip stocks that surged too much recently
            if surge_days>0:
                filtered=[]
                for c,s in cand:
                    di_c=idx[c].get(dt)
                    if di_c is None or di_c<surge_days:
                        filtered.append((c,s)); continue
                    px_now=stocks[c]['close'][di_c]
                    px_past=stocks[c]['close'][di_c-surge_days]
                    if px_past>0:
                        surge=(px_now-px_past)/px_past
    ...
    return {'sh':sh,'tr':tr*100,'cagr':cagr*100,'mdd':mdd*100,'calmar':cm,'nt':t_n,'wr':wr*100,'neg':neg,'ann':ann,'surge_skip':surge_skip}

# BASELINE
r_base=backtest(0.18,21,3,8,0.0,0,0)
print('='*95)
print('  NO-SURGE FILTER: skip stocks that already rallied before signal')
print('  Base: K=1.0 LB=21 Trail=18% Pos=8 Sec=Relaxed')
print('='*95)
print('  {:<30s} {:>7s} {:>8s} {:>6s} {:>7s} {:>6s} {:>5s} {:>4s} {:>6s}'.format(
    'Filter','Sharpe','Return%','MDD%','Calmar','Trades','Win%','Neg','Skipped'))
print('  '+'-'*80)
print('  {:<30s} {:>7.4f} {:>7.1f}% {:>5.1f}% {:>7.3f} {:>6d} {:>4.0f}% {:>4d} {:>6s}  [BASELINE]'.format(
    'BASELINE (no filter)',r_base['sh'],r_base['tr'],r_base['mdd'],r_base['calmar'],r_base['nt'],r_base['wr'],r_base['neg'],'—'))

best=None
best_label=''
for surge_d in [5,7,10,14]:
    for surge_t in [0.05,0.08,0.10,0.12,0.15,0.20]:
        r=backtest(0.18,21,3,8,0.0,surge_d,surge_t)
        if r:
            tag=''
            if r['sh']>r_base['sh']+0.005: tag=' SH+'
            if r['mdd']<r_base['mdd']-1: tag+=' DD-'
            if best is None or r['sh']>best['sh']: best=r;best_label='Rise>{}% in {}d'.format(int(surge_t*100),surge_d)
            print('  {:<30s} {:>7.4f} {:>7.1f}% {:>5.1f}% {:>7.3f} {:>6d} {:>4.0f}% {:>4d} {:>6d}{}'.format(
                'Rise>{}% in {}d'.format(int(surge_t*100),surge_d),r['sh'],r['tr'],r['mdd'],r['calmar'],r['nt'],r['wr'],r['neg'],r['surge_skip'],tag))

if best:
    print('\n'+'='*95)
    print('  BEST NO-SURGE: {}'.format(best_label))
    print('  S={:.4f} R={:.1f}% MDD={:.1f}% ({:+.0f}% vs base) CM={:.3f} Trd={:d} Win={:.0f}% Skipped={:d}'.format(
        best['sh'],best['tr'],best['mdd'],best['mdd']-r_base['mdd'],best['calmar'],best['nt'],best['wr'],best['surge_skip']))
