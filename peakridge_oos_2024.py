...
cd_full=get_common_dates(stocks)
cd=[d for d in cd_full if d>='2024-01-01']
print('[DATA] %d stocks, %d days (2024.01 ~ %s)' % (len(stocks),len(cd),cd[-1]))
...
def backtest(factor,trail,rebal,max_same_sector,max_pos,factor_floor):
    ...
    for di,dt in enumerate(cd):
        for code,p in list(pos.items()):
            ...
            if px<=p['peak']*(1-trail):
                ...
                trades.append({'code':code,'bd':p['bd'],'sd':dt,'ret':(sp-p['bp'])/p['bp'] if p['bp']>0 else 0,'exit':'trail'})
                del pos[code]
        if di%rebal==0:
            ...
            for code in list(pos.keys()):
                if code not in top_codes:
                    ...
                    trades.append({'code':code,'bd':pos[code]['bd'],'sd':dt,'ret':(sp-pos[code]['bp'])/pos[code]['bp'] if pos[code]['bp']>0 else 0,'exit':'rebal'})
                    del pos[code]
    ...
    for code,p in list(pos.items()):
        if code in idx and ld in idx[code]:
            ...
            sp=px*(1-SLIP-S_FEE-STAX);cash+=p['shares']*sp
            trades.append({'code':code,'bd':p['bd'],'sd':ld,'ret':(sp-p['bp'])/p['bp'] if p['bp']>0 else 0,'exit':'final'})
    ...
    return {'sh':sh,'tr':tr*100,'cagr':cagr*100,'mdd':mdd*100,'calmar':cm,'nt':len(trades),'wr':wr*100,'neg':neg,'ann':ann}
            ...
            for mp_ in [3,5,8]:
                for sec_ in [1,3]:
                    for floor_ in [0,0.8]:
                        ...
                        if r:
                            results.append({'K':K_,'LB':LB_,'Trail':tr_,'Pos':mp_,'Sec':sec_,'Floor':floor_,'S':r['sh'],'R':r['tr'],'CAGR':r['cagr'],'MDD':r['mdd'],'CM':r['calmar'],'Trd':r['nt'],'Win':r['wr'],'Neg':r['neg']})
                        if cnt%400==0: print('  [%d/%d]'%(cnt,total))

print('\nDone. %d results.' % len(results))
results.sort(key=lambda x:x['S'],reverse=True)

# --- Full-period best on OOS ---
r_global=backtest(FAC[(1.5,14)],0.30,21,1,5,0.8)
print('\n'+'='*90)
print('  Full-period BEST (K=1.5 LB=14 Trail=30% Pos=5 Sec=strict Floor=0.8) on 2024-2026:')
print('  Sharpe=%.4f | Ret=%.1f%% | CAGR=%.1f%% | MDD=%.1f%% | CM=%.3f | Trd=%d | Win=%.0f%% | NegY=%d' % (
    r_global['sh'],r_global['tr'],r_global['cagr'],r_global['mdd'],r_global['calmar'],r_global['nt'],r_global['wr'],r_global['neg']))
print('  Annual: '+' '.join('%s:%+.1f%%'%(y,r_global['ann'][y]) for y in sorted(r_global['ann'].keys())))

# --- OOS best ---
print('\n'+'='*90)
print('  TOP 20 for 2024-2026 OOS')
print('='*90)
...
for i,r in enumerate(results[:20]):
    print('  %4d %4.1f %4d %4.0f%% %4d %4d %5.1f %7.4f %7.1f %5.1f %5.1f %6.3f %4.0f%%' % (
        i+1,r['K'],r['LB'],r['Trail']*100,r['Pos'],r['Sec'],r['Floor'],r['S'],r['R'],r['CAGR'],r['MDD'],r['CM'],r['Win']))

# Best OOS detail
best=results[0]
r_best=backtest(FAC[(best['K'],best['LB'])],best['Trail'],21,best['Sec'],best['Pos'],best['Floor'])
print('\n  OOS Best: K=%.1f LB=%d Trail=%d%% Pos=%d Sec=%d Floor=%.1f' % (
    best['K'],best['LB'],best['Trail']*100,best['Pos'],best['Sec'],best['Floor']))
print('  Sharpe=%.4f | Ret=%.1f%% | MDD=%.1f%% | CM=%.3f' % (best['S'],best['R'],best['MDD'],best['CM']))
if r_best:
    print('  Annual: '+' '.join('%s:%+.1f%%'%(y,r_best['ann'][y]) for y in sorted(r_best['ann'].keys())))

# Param importance
print('\n  PARAM IMPORTANCE (OOS avg Sharpe):')
for param in ['K','LB','Pos','Sec','Floor','Trail']:
    ...
    print('  %s:'%param,' '.join('%s:%.3f'%(k,avgs[k]) for k in sorted(avgs.keys())))
