...
for c in codes:
    ...
    for i in range(len(bars)):
        ...
        if not math.isnan(mf[i]) and not math.isnan(ms[i]) and ms[i]>0:
            ...
            trnd[d]=mf[i-1]>ms[i-1] and sk if i>0 else False;rat[d]=mf[i-1]/ms[i-1] if i>0 and ms[i-1]>0 else 1.0
        else:trnd[d]=False;rat[d]=1.0
        abv[d]=not math.isnan(m60[i-1]) and cl[i-1]>m60[i-1] if i>0 else False;hgh[d]=hi[i]
