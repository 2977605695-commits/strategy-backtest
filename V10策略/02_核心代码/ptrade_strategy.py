...
def handle_data(context, data):
    ...
    try:
        ...
        if strategy.position:
            ...
            # ★ 终极版: 传入ATR用于自适应Trail
            atr_value = etf_signals.get('atr', {}).get(pos_code, {}).get(date_str, 0)
            exit_reason = strategy.check_exit(current_price, date_str, trend_on, is_bear, atr_value)
...
def _update_signals(context, date_str):
    ...
    etf_signals = {'trend': {}, 'ratio': {}, 'above_filter': {}, 'above_ma60': {}, 'atr': {}, 'highs': {}}
    index_signals = {}
    bar_data_cache = {}
    ...
    for code in config['pool']['full']:
        ...
        etf_signals['above_filter'][code] = sig.get('above_filter', sig.get('above_ma60', {}))
        etf_signals['above_ma60'][code] = sig.get('above_ma60', sig.get('above_filter', {}))
        etf_signals['atr'][code] = sig.get('atr', {})
