"""降换手测试: 在原版最优参数基础上, 加换股门槛减少无谓调仓.
不动MA/Trail等核心参数, 只改'是否换股'的判断逻辑.
测试: 1.换股门槛(ratio差) 2.最小持仓N天 3.信号确认N天.
"""
import json, os, math
...
def run(switch_threshold=0.0, min_hold=0, confirm_days=1, use_bear_mh=True):
    """
    min_hold: 额外最小持仓天数 (叠加在熊市MH之上, 牛市也强制)
    confirm_days: 信号确认天数 (趋势信号连续N天才生效) - 1=原版
    use_bear_mh: 是否启用熊市MH=7 (原版True)
    """
    cash=INIT;pos=None;shares=0.0;bp=0.0;peak=0.0;entry_d='';entry_date=None;dvs=[]
    pool_mode='all';rolling_pnl=[];n_trades=0
    # 信号确认: 跟踪每只ETF的趋势信号连续天数
    trend_streak={c:0 for c in codes}
    for d in all_dates:
        avail=[c for c in codes if fd[c]<=d];dt_obj=datetime.strptime(d,'%Y-%m-%d')
        # 更新连续天数
        for c in avail:
            if all_trnd[c].get(d,False):trend_streak[c]=trend_streak.get(c,0)+1
            else:trend_streak[c]=0
        is_bear=not index_slope.get('510300',{}).get(d,False)
        cur_trail=TRAIL_BEAR if is_bear else TRAIL_BULL
        bear_mh = BEAR_MH if (is_bear and use_bear_mh) else 0
        cur_mh=max(bear_mh, min_hold)
        if pos:
            bar=dm[pos].get(d)
            if bar:
                ...
                if px_raw<=peak*(1-cur_trail):er='trail'
                    ...
                    hold_days=(dt_obj-entry_date).days if entry_date else 999
                    if hold_days>=cur_mh:er='off'
                if er:
                    n_trades+=1
        ...
        if not pos and cash>0:
            ...
            for c in avail:
                ...
                if trend_streak.get(c,0)<confirm_days:continue  # 信号确认
                ...
                if PULLBACK_MIN>0:
                    ...
                    for lb in range(1,21):
                        pdi=di-lb
                        if 0<=pdi<len(date_list):hi20=max(hi20,etf_highs[c].get(date_list[pdi],0))
    ...
    return {'sh':sh,'tr':tr,'mdd':mdd,'n_trades':n_trades}

# 基准
base=run()
print('='*100)
print('  降换手测试 (原版参数不变, 只改换股逻辑)')
print('='*100)
print('  基准(原版): 夏普=%.3f 收益=%+.0f%% 回撤=%.1f%% 笔数=%d\n' % (base['sh'],base['tr']*100,base['mdd']*100,base['n_trades']))

print('【测试1: 最小持仓天数 (牛市也强制持仓N天, 减少折腾)】')
print('  %-12s %8s %10s %8s %8s %s' % ('最小持仓','夏普','收益','回撤','笔数','vs基准'))
print('  '+'-'*70)
for mh in [0,2,3,5,7,10]:
    r=run(min_hold=mh)
    dsh=r['sh']-base['sh']
    print('  %-12d %8.3f %+9.0f%% %7.1f%% %7d %+.3f' % (mh,r['sh'],r['tr']*100,r['mdd']*100,r['n_trades'],dsh))

print('\n【测试2: 信号确认天数 (趋势连续N天才买入, 减少假信号)】')
print('  %-12s %8s %10s %8s %8s %s' % ('确认天数','夏普','收益','回撤','笔数','vs基准'))
print('  '+'-'*70)
for cd in [1,2,3,4,5]:
    r=run(confirm_days=cd)
    dsh=r['sh']-base['sh']
    print('  %-12d %8.3f %+9.0f%% %7.1f%% %7d %+.3f' % (cd,r['sh'],r['tr']*100,r['mdd']*100,r['n_trades'],dsh))

print('\n【测试3: 组合 - 最小持仓3天 + 信号确认2天】')
r=run(min_hold=3,confirm_days=2)
print('  夏普=%.3f 收益=%+.0f%% 回撤=%.1f%% 笔数=%d (基准夏普%.3f 笔数%d)' % (
    r['sh'],r['tr']*100,r['mdd']*100,r['n_trades'],base['sh'],base['n_trades']))

print('\n【测试4: 组合 - 最小持仓5天 + 信号确认3天】')
r=run(min_hold=5,confirm_days=3)
print('  夏普=%.3f 收益=%+.0f%% 回撤=%.1f%% 笔数=%d (基准夏普%.3f 笔数%d)' % (
    r['sh'],r['tr']*100,r['mdd']*100,r['n_trades'],base['sh'],base['n_trades']))
