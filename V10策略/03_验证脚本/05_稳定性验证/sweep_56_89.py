"""56池 + 89池 全量扫参 (贪心法), 真实ETF费率(万2.5+0.03滑点).
看新标的池能否通过调参提升, 以及合并89池最优参数.
"""
import json, os, math
...
SL_MA=8; BULL_MH=0
SLIP=0.0003; COMM=0.00025; STAMP=0.0  # 真实ETF费率
    ...
    for code in codes:
        for dd in ['data_new','data']:
            ...
            if os.path.exists(p):
                ...
                if bars:
                    etfs[code]={'name':d['name'],'bars':bars};break
    return etfs
...
INDEX_NEW={'510310':'沪深300','159901':'深100','512100':'中证1000','510050':'上证50','512760':'半导体50','159994':'5G','513500':'标普500','511220':'城投债','515080':'红利','512170':'医疗'}
# 56池牛熊判断用510310(原版510300不在新池)
BEAR_CODE_56='510310'
...
def run(etfs, sig, def_codes, p):
    ...
    bear_code=BEAR_CODE_56 if BEAR_CODE_56 in index_slope else next(iter(index_slope),None)
    for d in all_dates:
        ...
        if pos:
            ...
            if bar and bar['valid']:
                ...
                if er:
                    ...
                    if pool_mode=='defensive' and sum(1 for c2 in INDEX_NEW if index_slope.get(c2,{}).get(d,False))>=need_n:pool_mode='all'
...
def sweep_pool(codes_pool, def_codes, label):
    etfs=load_pool(codes_pool)
    base_p={'f_ma':6,'s_ma':15,'sl_lb':4,'trail_bull':0.03,'trail_bear':0.06,'mh_bear':7,'pullback':0.05,'need_n':7}
    cur=dict(base_p)
    def sweep(name, key, vals, rebuild=False):
        results=[]
        for v in vals:
            p=dict(cur);p[key]=v
            sig=precompute(etfs,p.get('f_ma',6),p.get('s_ma',15),p.get('sl_lb',4)) if rebuild else SIG_DEFAULT
            r=run(etfs,sig,def_codes,p)
            results.append((v,r['sh'],r['tr'],r['mdd']))
        results.sort(key=lambda x:-x[1])
        cur[key]=results[0][0]
        return results[0]
    SIG_DEFAULT=precompute(etfs,6,15,4)
    base=run(etfs,SIG_DEFAULT,def_codes,base_p)
    print('='*100)
    print('  %s 全量扫参 | 真实ETF费率(万2.5+0.03滑点)' % label)
    print('='*100)
    ...
    final=run(etfs,final_sig,def_codes,cur)
    ...
    if changed:
        ...
        print('  ★ 原版参数已是最优, 无需调整')
    print()
    return base,final,cur

# ===== 56池扫参 =====
b56,f56,p56=sweep_pool(NEW_56, DEF_NEW, '56池(新标的)')

# ===== 89池扫参 (合并, 用原版检测池) =====
ETF_89=sorted(set(ETF_33+NEW_56))
DEF_89=sorted(set(['518880','159937','518800','511010','511260','510880','512890','510050']+DEF_NEW))
# 89池用原版检测池(含510300)
import sys
# 临时改INDEX_NEW为原版检测池用于89池
INDEX_OLD={'510300':'HS300','159915':'创业板','588000':'科创50','510050':'上证50','515880':'通信ETF','512480':'半导体ETF','513100':'纳指ETF','511260':'十年国债','512890':'红利低波','159992':'创新药ETF'}
# 重写precompute的INDEX引用 - 直接用全局变量
print('(89池使用原版检测池, 含510300)')
