"""Slimmed ETF Pool (18 ETFs) vs Original (33 ETFs) · Head-to-Head"""
import json,os,math
...
# Full 33 pool
ETF_FULL=['159782','588380','588870','588080','588300','518800','589720','588890','588170',
...
# Slim 18 pool (removed: never-traded + negative + redundant)
ETF_SLIM=[
    # Tech core (6)
    '588170','515880','515050','512480','588200','159509',
    # Gold (2)
    '159937','518800',
    # Bond/Defensive (3)
    '511260','510880','512890',
    # Offshore (3)
    '513100','513050','513180',
    # Healthcare (2)
    '159992','512010',
    # Broad (2 - keep 1 KC50, 1 ChiNext/HS300 for defense)
    '588080','159915',
]

# Defense pool (same, but slimmed)
DEF_SLIM=['159937','518800','511010','511260','510880','512890','510050']
DEF_FULL=['518880','159937','518800','511010','511260','510880','512890','510050']
...
def run(etfs,dcodes):
    ...
    pool_mode='all';rolling_pnl=[];switched_date='';dvs=[]
    ...
    for d in all_dates:
        ...
        if not pos and cash>0:
            cands=[]
            for c in avail:
                if pool_mode=='defensive' and c not in dcodes:continue
...
# ===== RUN =====
# Cache needs module-level
all_trnd={};all_ratio={};above_ma60={};etf_highs={};index_slope={}

print('='*110)
print('  SLIM (18 ETFs) vs FULL (33 ETFs) · Head-to-Head')
print('='*110)

# Full pool
etfs_full=load(ETF_FULL)
precompute(etfs_full)
sh_f,tr_f,mdd_f,ar_f,np_f,wr_f,yr_f=run(etfs_full,DEF_FULL)

# Slim pool
all_trnd={};all_ratio={};above_ma60={};etf_highs={};index_slope={}
etfs_slim=load(ETF_SLIM)
precompute(etfs_slim)
sh_s,tr_s,mdd_s,ar_s,np_s,wr_s,yr_s=run(etfs_slim,DEF_SLIM)

print()
print('  %-15s %8s %8s %7s %6s %5s %5s %7s %7s %7s %7s %7s %7s'%('Pool','ETFs','S','Ret','DD','Ann','Trd','Win','2020','2021','2022','2025','2026'))
print('  '+'-'*115)
for label,sh,tr,mdd,ar,np,wr,yr,ne in[('FULL (33)',sh_f,tr_f,mdd_f,ar_f,np_f,wr_f,yr_f,33),('SLIM (18)',sh_s,tr_s,mdd_s,ar_s,np_s,wr_s,yr_s,18)]:
    cum=1.0
    for y_ in['2020','2021','2022','2023','2024','2025','2026']:cum*=1+yr.get(y_,0)/INIT/100
    print('  %-15s %8d %8.3f %7.2f%% %5.2f%% %5.2f%% %5d %4.0f%% %+7.1f%% %+7.1f%% %+7.1f%% %+7.0f%% %+7.0f%%'%(
        label,ne,sh,tr*100,mdd*100,ar*100,np,wr*100,
        yr.get('2020',0)/INIT*100,yr.get('2021',0)/INIT*100,
        yr.get('2022',0)/INIT*100,yr.get('2025',0)/INIT*100,yr.get('2026',0)/INIT*100))
